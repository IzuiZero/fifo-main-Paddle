...
import cv2  # OpenCV as a replacement for scipy.misc
import paddle.vision.transforms as transforms
import paddle.vision.transforms.functional as F

class Pairedcityscapes(Dataset):
    colors = [  
        [128, 64, 128],
        [244, 35, 232],
        [70, 70, 70],
    ...
    def __getitem__(self, index):
        ...
        src_image = F.crop(src_image, i, j, h, w)
        trg_image = F.crop(trg_image, i, j, h, w)
        label = F.crop(label, i, j, h, w)

        if random.random() > 0.5:
            src_image = F.hflip(src_image)
            trg_image = F.hflip(trg_image)
            label = F.hflip(label)
    ...
    def _apply_transform(self, img1, img2, lbl, scale=(0.7, 1.3)):
        ...
        tsfrms = []
        tsfrms.append(transforms.Resize(size=(int(H * scale), int(W * scale))))
...
if __name__ == '__main__':
    src_root = "./data/src"
    trg_root = "./data/trg"
    src_list_path = "./data/src_list.txt"
    trg_list_path = "./data/trg_list.txt"
    dst = Pairedcityscapes(src_root, trg_root, src_list_path, trg_list_path, mean=(128, 128, 128))
    trainloader = DataLoader(dst, batch_size=4, shuffle=True)
    for i, data in enumerate(trainloader):
        src_imgs, trg_imgs, labels, sizes, src_names, trg_names = data
        if i == 0:
            img = paddle.vision.utils.make_grid(src_imgs).numpy()
