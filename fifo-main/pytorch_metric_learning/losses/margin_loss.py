...
class MarginLoss(BaseMetricLossFunction):
    ...
    def compute_loss(self, embeddings, labels, indices_tuple):
        ...
        beta = self.beta if len(self.beta) == 1 else self.beta[labels.numpy()[anchor_idx]]
        beta = c_f.to_device(beta, device=embeddings.device, dtype=embeddings.dtype)

        mat = self.distance(embeddings)

        d_ap = mat.numpy()[anchor_idx, positive_idx]
        d_an = mat.numpy()[anchor_idx, negative_idx]

        pos_loss = paddle.nn.functional.relu(
            self.distance.margin(d_ap, beta) + self.margin
        )
        neg_loss = paddle.nn.functional.relu(
            self.distance.margin(beta, d_an) + self.margin
        )

        num_pos_pairs = paddle.sum(pos_loss > 0.0)
        num_neg_pairs = paddle.sum(neg_loss > 0.0)
    ...
    def initialize_beta(self, beta, num_classes):
        self.beta = paddle.to_tensor([float(beta)])
        if num_classes:
            self.beta = paddle.ones(num_classes) * self.beta
        if self.learn_beta:
            self.beta = paddle.create_parameter(shape=self.beta.shape, dtype=self.beta.dtype, default_initializer=paddle.nn.initializer.Assign(self.beta))
