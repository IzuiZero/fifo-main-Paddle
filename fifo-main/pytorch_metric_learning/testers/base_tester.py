...
class BaseTester:
    def __init__(
        ...
        self.data_device = (
            paddle.device.get_device() if data_device is None else data_device
        )
        self.dtype = dtype
    ...
    def maybe_normalize(self, embeddings):
        if self.pca:
            for_pca = c_f.paddle_standard_scaler(embeddings)
            embeddings = stat_utils.run_pca(for_pca, self.pca)
        if self.normalize_embeddings:
            embeddings = paddle.nn.functional.normalize(embeddings)
        return embeddings
    ...
    def compute_all_embeddings(self, dataloader, trunk_model, embedder_model):
        ...
        with paddle.no_grad():
            for i, data in enumerate(tqdm.tqdm(dataloader)):
                ...
                if i == 0:
                    labels = paddle.zeros(
                        (len(dataloader.dataset), label.shape[1]),
                        device=self.data_device,
                        dtype=label.dtype,
                    )
                    all_q = paddle.zeros(
                        (len(dataloader.dataset), q.shape[1]),
                        device=self.data_device,
                        dtype=q.dtype,
                    )
                e = s + q.shape[0]
    ...
    def maybe_combine_splits(self, embeddings_and_labels, splits):
        ...
        curr_embeddings = paddle.concat(eee, axis=0)
        curr_labels = paddle.concat(lll, axis=0)
    ...
    def test(
        ...
        if self.end_of_testing_hook:
            ...
            c_f.LOGGER.info(self.all_accuracies)
