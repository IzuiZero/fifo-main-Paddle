...
def pos_inf(dtype):
    return paddle.finfo(dtype).max


def neg_inf(dtype):
    return paddle.finfo(dtype).min


def small_val(dtype):
    return paddle.finfo(dtype).tiny
...
def shift_indices_tuple(indices_tuple, batch_size):
    ...
    Args:
        indices_tuple is a tuple with paddle.Tensor
...
def safe_random_choice(input_data, size):
    ...
    Args:
        input_data is a sequence, like a paddle tensor, numpy array,
...
def unslice_by_n(input_tensors):
    n = len(input_tensors)
    rows, cols = input_tensors[0].shape
    output = paddle.zeros((rows * n, cols), place=input_tensors[0].place)
    for i in range(n):
...
def get_train_dataloader(dataset, batch_size, sampler, num_workers, collate_fn):
    if isinstance(sampler, paddle.io.BatchSampler):
...
def try_paddle_operation(paddle_op, input_val):
    return paddle_op(input_val) if isinstance(input_val, paddle.Tensor) else input_val
...
def modelpath_creator(folder, basename, identifier, extension=".pdparams"):
...
def save_model(model, filepath):
    if any(
        isinstance(model, x)
        for x in [paddle.DataParallel]
    ):
        paddle.save(model._layers.state_dict(), filepath)
    else:
...
def load_model(model_def, model_filename, device):
    try:
        model_def.set_state_dict(paddle.load(model_filename))
    except KeyError:
        # original saved file with DataParallel
        state_dict = paddle.load(model_filename)
        ...
        # load params
        model_def.set_state_dict(new_state_dict)
...
def latest_version(folder, string_to_glob="trunk_*.pdparams", best=False):
    items = glob.glob(os.path.join(folder, string_to_glob))
    if items == []:
        return (0, None)
    model_regex = (
        regex_wrapper("best[0-9]+\.pdparams$") if best else regex_wrapper("[0-9]+\.pdparams$")
    )
    epoch_regex = regex_wrapper("[0-9]+\.pdparams$")
...
def paddle_arange_from_size(input, size_dim=0):
    return paddle.arange(input.shape[size_dim], place=input.place)


class PaddleInitWrapper:
...
class EmbeddingDataset(paddle.io.Dataset):
...
def paddle_all_from_dim_to_end(x, dim):
    return paddle.all(x.reshape((*x.shape[:dim], -1)), axis=-1)


def paddle_standard_scaler(x):
    mean = paddle.mean(x, axis=0)
    std = paddle.std(x, axis=0)
    return (x - mean) / std


def to_dtype(x, tensor=None, dtype=None):
    if not paddle.get_flags("FLAGS_use_autocast"):
...
def to_device(x, tensor=None, device=None, dtype=None):
    dv = device if device is not None else tensor.place
    if x.place != dv:
        x = x.place(dv)
