...
class BaseReducer(ModuleWithRecords):
    ...
    def zero_loss(self, embeddings):
        return paddle.sum(embeddings * 0)

    def input_is_zero_loss(self, losses):
        if (not isinstance(losses, paddle.Tensor)) and (losses == 0):
            return True
        return False
    ...
    def assert_sizes_element(self, losses, loss_indices):
        assert isinstance(losses, paddle.Tensor)
        assert isinstance(loss_indices, paddle.Tensor)
    ...
    def assert_sizes_pair(self, losses, loss_indices):
        ...
        assert all(isinstance(x, paddle.Tensor) for x in loss_indices)
    ...
    def set_losses_size_stat(self, losses):
        if self.collect_stats:
            ...
            if not isinstance(losses, paddle.Tensor) or losses.ndim == 0:
                self.losses_size = 1
            else:
