...
EQUALITY = paddle.equal


def maybe_get_avg_of_avgs(accuracy_per_sample, sample_labels, avg_of_avgs):
    if avg_of_avgs:
        unique_labels = paddle.unique(sample_labels, axis=0)
        mask = c_f.paddle_all_from_dim_to_end(
            sample_labels == unique_labels.unsqueeze(1), 2
        )
        mask = paddle.t(mask)
        acc_sum_per_class = paddle.sum(accuracy_per_sample.unsqueeze(1) * mask, axis=0)
        mask_sum_per_class = paddle.sum(mask, axis=0)
        average_per_class = acc_sum_per_class / mask_sum_per_class
        return paddle.mean(average_per_class).item()
    return paddle.mean(accuracy_per_sample).item()
...
def get_relevance_mask(
    ...
    relevance_mask = paddle.zeros(shape=shape, dtype='bool', place=gt_labels.place)

    for label, count in zip(*label_counts):
        matching_rows = paddle.nonzero(
            c_f.paddle_all_from_dim_to_end(gt_labels == label, 1)
        )[:, 0]
...
def r_precision(
    ...
    matches_per_row = paddle.sum(same_label * relevance_mask, axis=1)
    max_possible_matches_per_row = paddle.sum(relevance_mask, axis=1)
    accuracy_per_sample = (
        c_f.to_dtype(matches_per_row, dtype='float64')
...
def mean_average_precision(
    ...
    device = gt_labels.place
    num_samples, num_k = knn_labels.shape[:2]
    relevance_mask = (
        paddle.ones((num_samples, num_k), dtype='bool', place=device)
        if relevance_mask is None
        else relevance_mask
    )
    is_same_label = label_comparison_fn(gt_labels, knn_labels)
    equality = is_same_label * relevance_mask
    cumulative_correct = paddle.cumsum(equality, axis=1)
    k_idx = paddle.arange(1, num_k + 1, place=device).repeat(num_samples, 1)
    precision_at_ks = (
        c_f.to_dtype(cumulative_correct * equality, dtype='float64') / k_idx
    )
    summed_precision_per_row = paddle.sum(precision_at_ks * relevance_mask, axis=1)
    if at_r:
        max_possible_matches_per_row = paddle.sum(relevance_mask, axis=1)
    else:
        max_possible_matches_per_row = paddle.sum(equality, axis=1)
        max_possible_matches_per_row = paddle.where(max_possible_matches_per_row == 0, paddle.ones_like(max_possible_matches_per_row), max_possible_matches_per_row)
...
def get_lone_query_labels(
    ...
    if embeddings_come_from_same_source:
        ...
        lone_condition = (
            match_counts - c_f.to_dtype(label_matches_itself, dtype='int64') <= 0
        )
    else:
    ...
    if len(lone_query_labels) > 0:
        ...
        not_lone_query_mask = ~paddle.any(
            c_f.paddle_all_from_dim_to_end(comparison, 2), axis=1
        )
    else:
        not_lone_query_mask = paddle.ones(
            query_labels.shape[0], dtype='bool', place=query_labels.place
        )
...
class AccuracyCalculator:
    ...
    def get_cluster_labels(self, query, query_labels, **kwargs):
        num_clusters = len(paddle.unique(query_labels.flatten()))
    ...
    def determine_k(
        ...
        if self.k == "max_bin_count":
            return paddle.max(bin_counts).item() - self_count
        if self.k is None:
