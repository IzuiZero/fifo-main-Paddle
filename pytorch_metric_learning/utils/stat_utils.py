...
def try_gpu(cpu_index, reference_embeddings, test_embeddings, k):
    ...
    if gpus_are_available:
        max_k_for_gpu = 1024 if float(paddle.__version__) < 9.5 else 2048
        if k <= max_k_for_gpu:
...
def get_knn(
    ...
    distances = paddle.to_tensor(distances, dtype='float32', place=device)
    indices = paddle.to_tensor(indices, dtype='int64', place=device)
    if embeddings_come_from_same_source:
...
def run_kmeans(x, nmb_clusters):
    ...
    return paddle.to_tensor([int(n[0]) for n in idxs], dtype='int64', place=device)
...
def run_pca(x, output_dimensionality):
    ...
    return paddle.to_tensor(mat.apply_py(x), dtype='float32', place=device)
