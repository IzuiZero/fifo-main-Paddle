...
class MaximumLossMiner(BaseSubsetBatchMiner):
    ...
    def mine(self, embeddings, labels, *_):
        ...
        for i in range(self.num_trials):
            rand_subset_idx = np.random.choice(
                len(embeddings), size=self.output_batch_size, replace=False
            )
            rand_subset_idx = paddle.to_tensor(rand_subset_idx)
            ...
            losses.append(self.loss(curr_embeddings, curr_labels, indices_tuple).numpy().item())
