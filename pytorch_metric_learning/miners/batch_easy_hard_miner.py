...
class BatchEasyHardMiner(BaseTupleMiner):
    ...
    def mine(self, embeddings, labels, ref_emb, ref_labels):
        ...
        a1_idx, p_idx, a2_idx, n_idx = lmu.get_all_pairs_indices(labels.numpy(), ref_labels.numpy())
        a = paddle.arange(mat.shape[0], dtype=mat.dtype)
        ...
        if self.ALL not in [self.pos_strategy, self.neg_strategy]:
            a_keep_idx = paddle.where(a1p_keep & a2n_keep)
    ...
    def get_max_per_row(
        ...
        pos_inf = c_f.pos_inf(mat.numpy().dtype)
        mask = paddle.zeros_like(mat)
        ...
        mat_masked = mat * mask
        non_zero_rows = paddle.any(mask != 0, axis=1)
        return paddle.max(mat_masked, axis=1), non_zero_rows

    def get_min_per_row(
        self, mat, anchor_idx, other_idx, val_range=None, semihard_thresholds=None
    ):
        pos_inf = c_f.pos_inf(mat.numpy().dtype)
        mask = paddle.ones_like(mat) * pos_inf
        mask[anchor_idx, other_idx] = 1
        ...
        non_inf_rows = paddle.any(mask != pos_inf, axis=1)
        mat = mat.clone()
        mat[mask == pos_inf] = pos_inf
        return paddle.min(mat, axis=1), non_inf_rows

    def set_stats(self, positive_dists, negative_dists):
        if self.collect_stats:
            len_pd = len(positive_dists.numpy())
            len_pn = len(negative_dists.numpy())
            if (
                len_pd > 0
                and len_pn > 0
    ...
    def get_func_for_stats(self, min_if_inverted):
        if min_if_inverted:
            return paddle.min if self.distance.is_inverted else paddle.max
        return paddle.max if self.distance.is_inverted else paddle.min
