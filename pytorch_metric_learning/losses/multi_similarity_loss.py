...
class MultiSimilarityLoss(GenericPairLoss):
    ...
    def _compute_loss(self, mat, pos_mask, neg_mask):
        ...
        pos_loss = (1.0 / self.alpha) * lmu.logsumexp(
            self.alpha * pos_exp, keep_mask=pos_mask.astype('float32'), add_one=True
        )
        neg_loss = (1.0 / self.beta) * lmu.logsumexp(
            self.beta * neg_exp, keep_mask=neg_mask.astype('float32'), add_one=True
        )
        return {
            "loss": {
                "losses": pos_loss + neg_loss,
                "indices": c_f.paddle_arange_from_size(mat.shape[0]),
