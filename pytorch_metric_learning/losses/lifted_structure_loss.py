...
class LiftedStructureLoss(GenericPairLoss):
    ...
    def _compute_loss(self, pos_pairs, neg_pairs, indices_tuple):
        ...
        if len(a1) > 0 and len(a2) > 0:
            ...
            loss_per_pos_pair = F.relu(loss_per_pos_pair) ** 2
...
class GeneralizedLiftedStructureLoss(GenericPairLoss):
    ...
    def _compute_loss(self, mat, pos_mask, neg_mask):
        ...
        pos_loss = lmu.logsumexp(
            remaining_pos_margin, keep_mask=paddle.cast(pos_mask.bool(), 'float32'), add_one=False
        )
        neg_loss = lmu.logsumexp(
            remaining_neg_margin, keep_mask=paddle.cast(neg_mask.bool(), 'float32'), add_one=False
        )
        return {
            "loss": {
                "losses": F.relu(pos_loss + neg_loss),
