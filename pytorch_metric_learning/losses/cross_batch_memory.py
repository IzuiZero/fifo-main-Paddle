...
class CrossBatchMemory(ModuleWithRecords):
    def __init__(self, loss, embedding_size, memory_size=1024, miner=None, **kwargs):
        ...
        self.embedding_memory = paddle.zeros([self.memory_size, self.embedding_size])
        self.label_memory = paddle.zeros([self.memory_size], dtype='int64')
    ...
    def forward(self, embeddings, labels, indices_tuple=None, enqueue_idx=None):
        ...
        if enqueue_idx is not None:
            mask = paddle.zeros([len(embeddings)], dtype='bool', place=device)
        ...
        combined_embeddings = paddle.concat([embeddings, E_mem], axis=0)
        combined_labels = paddle.concat([labels, L_mem], axis=0)
    ...
    def add_to_memory(self, embeddings, labels, batch_size):
        self.curr_batch_idx = (
            paddle.arange(
                self.queue_idx, self.queue_idx + batch_size, dtype='int64', place=labels.place
            )
            % self.memory_size
        )
    ...
    def create_indices_tuple(
        ...
        if input_indices_tuple is not None:
            ...
            indices_tuple = tuple(
                [
                    paddle.concat([x, c_f.to_device(y, x)], axis=0)
    ...
    def not_self_comparisons(self, a, p, s, e):
        curr_batch = paddle.any(p.unsqueeze(1) == self.curr_batch_idx, axis=1)
        a_c = a[curr_batch]
        p_c = p[curr_batch]
        p_c -= s
        if e <= s:
        ...
        without_self_comparisons[paddle.where(curr_batch)[0][a_c == p_c]] = False
