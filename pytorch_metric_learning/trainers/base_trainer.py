...
class BaseTrainer:
    ...
    def train(self, start_epoch=1, num_epochs=1):
        ...
        for self.epoch in range(start_epoch, num_epochs + 1):
            ...
            self.zero_losses()
            if self.end_of_epoch_hook and not self.end_of_epoch_hook(self):
                break
    ...
    def initialize_data_device(self):
        if self.data_device is None:
            self.data_device = paddle.set_device("gpu" if paddle.is_compiled_with_cuda() else "cpu")
